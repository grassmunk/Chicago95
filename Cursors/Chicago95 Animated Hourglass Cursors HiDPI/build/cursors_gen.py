...
# Scale cursors by this much when rendering. Useful for HiDPI.
scale = 2
...
def convert_cur_files(cursor_filename, output_file_name):
	scale_arg = str(scale * 100) + "%x" + str(scale*100) + "%"
...
for cursor in cursors:
	...
	if ext in ['.ico', '.cur']:
		...
		xhot = cursor_file_config['icon'][0]['rtIconDirEntry']['wPlanes'] * scale
		yhot = cursor_file_config['icon'][0]['rtIconDirEntry']['wBitCount'] * scale
		size = cursor_file_config['icon'][0]['rtIconDirEntry']['bHeight'] * scale
			...
			for sequence in ani_file_config['seq']:
				...
				for icon in ani_file_config['icon']:
					if icon['index'] == sequence:
						xhot = icon['rtIconDirEntry']['wPlanes'] * scale
						yhot = icon['rtIconDirEntry']['wBitCount'] * scale
						size = icon['rtIconDirEntry']['bHeight'] * scale
